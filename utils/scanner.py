...
class Token:
    ...
    @classmethod
    def from_output_entry(cls, output_entry: list, symbol_tables) -> None:
        if len(output_entry) == 1:
            return cls(
                get_token_name(token_id=output_entry[0]), type=TokenType.TERMINAL
            )
            ...
            return cls(entry.lexeme, entry.type)
        else:
    ...
    def print_formatted_scanner_output(self):
        ...
        formatted_output = tabulate(
            [
                [
                    output_entry,
                    Token.from_output_entry(output_entry, self.symbol_tables),
                ]
                for output_entry in self.scanner_output
            ],
